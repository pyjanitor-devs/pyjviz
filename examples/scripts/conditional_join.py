...
if __name__ == "__main__":
    ...
    s_df1 = pd.DataFrame({'id': [1,1,1,2,2,3],
                          'value_1': [2,5,7,1,3,4]})
    
    s_df2 = pd.DataFrame({'id': [1,1,1,1,2,2,2,3],
                          'value_2A': [0,3,7,12,0,2,3,1],
                          'value_2B': [1,5,9,15,1,4,6,3]})
    
    with pyjviz.Chain("c1") as c1:
        df1 = c1.pin(s_df1)
        res1 = df1.conditional_join(s_df2,
                                    ('id', 'id', "<"),
                                    df_columns = {'id':'df_id'},
    ...
    with c1:
        with pyjviz.Chain("c2", c1) as c2:
            res2 = df1.select_columns('value_1').conditional_join(
                c2.pin(s_df2).select_columns('val*'), # someday soon this line should work - df2.pin(c2).select_columns('val*'),
                ('value_1', 'value_2A', '>'),
                ('value_1', 'value_2B', '<'),
            )
