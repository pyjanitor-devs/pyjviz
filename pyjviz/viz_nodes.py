...
def rq_d(g, rq, init_bindings):
    ...
    if len(rq_res) != 1:
        ipdb.set_trace()
        raise Exception(f"rq_d: query result has length {len(rq_res)}, expected to be 1")
...
def make_table_popup_href(head_html, popup_output):
    ...
    if fstriplestore.triple_store.output_dir:
        ...
        temp_file = tempfile.NamedTemporaryFile(dir=temp_dir, suffix=".html", delete=False)
        with temp_file:
            popup_size = (800, 200)
...
def make_image_popup_href(image_b64, popup_output):
    img_html = "<img src='data:image/png;base64," + image_b64.toPython() + "'></img>"
...
class ObjStateGraphVizNode:
    ...
    def build_label(self):
        ...
        if cc_type == rdflib.URIRef("CCObjStateLabelDataFrame", base=fstriplestore.base_uri):
            self.df_shape = res_d.get('df_shape').toPython()
        elif cc_type == rdflib.URIRef("CCObjStateLabelSeries", base=fstriplestore.base_uri):
    ...
    def dump(self, out_fd):
        version_s = " " if self.version.toPython() == "0" else self.version.toPython()
        shape_s = None
        if self.df_shape:
